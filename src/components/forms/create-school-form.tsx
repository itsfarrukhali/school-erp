// src/components/forms/create-school-form.tsx
"use client";
...
export function CreateSchoolForm({ onSuccess }: CreateSchoolFormProps) {
  ...
  const onSubmit = async (data: CreateSchoolInput) => {
    ...
    try {
      const result = await schoolsApi.createSchool(data);

      if (result.success && result.data) {
        toast.success("School Created", {
          description: result.message || `${data.name} has been successfully created.`,
        });
        reset();
        onSuccess?.();
        router.refresh();
        ...
        // Handle API errors
        const errorMessage = result.message || "Failed to create school";
        toast.error("Error", {
      ...
      <CardHeader className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <CardDescription>
              Add a new school to the ERP system
            </CardDescription>
          </div>
        </div>
      </CardHeader>
